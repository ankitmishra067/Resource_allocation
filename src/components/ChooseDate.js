...
export default function ChooseDate() {
  ...
  return (
    ...
    <form className="login" >
      <div className="line">
        <h1>Set the project start date.</h1>
        <p>{employee.name},{employee.desc}</p>
        
      <input type="date" value={date} onChange={(e)=>setDate(e.target.value)} placeholder='Enter Date' /><br />
    
      <button onClick={(e)=>handleSubmit(employee._id,e)}>Set a date</button>
      </div>
    </form>
